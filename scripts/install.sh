...
usage() {
  cat <<'USAGE'
Miroclaw installer — one-click bootstrap

Usage:
  ./install.sh [options]

The installer builds Miroclaw, configures your provider and API key,
...
Environment:
  ...
  MIROCLAW_DOCKER_IMAGE      Docker image tag to build/run (default: miroclaw-bootstrap:local)
...
resolve_asset_url() {
  local asset_name="$1"
  local api_url="https://api.github.com/repos/morpheum-labs/miroclaw/releases"
...
install_prebuilt_binary() {
  ...
  asset_name="miroclaw-${target}.tar.gz"
  ...
  if [[ -z "$archive_url" ]]; then
    archive_url="https://github.com/morpheum-labs/miroclaw/releases/latest/download/${asset_name}"
  fi
  ...
  extracted_bin="$temp_dir/miroclaw"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name miroclaw -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable miroclaw binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/miroclaw"
  rm -rf "$temp_dir"

  step_ok "Installed pre-built binary to $install_dir/miroclaw"
...
prompt_api_key() {
  ...
  if [[ -n "$api_key_input" ]]; then
    ...
    warn "No API key entered — you can configure it later with miroclaw onboard"
    SKIP_ONBOARD=true
  fi
}
...
run_guided_installer() {
  ...
  echo
  echo -e "  ${BOLD_BLUE}${CRAB} Miroclaw Guided Installer${RESET}"
...
# Miroclaw configuration — generated by install.sh
# Edit this file or run 'miroclaw onboard' to reconfigure.
...
run_docker_bootstrap() {
  ...
  docker_image="${MIROCLAW_DOCKER_IMAGE:-miroclaw-bootstrap:local}"
  fallback_image="ghcr.io/morpheum-labs/miroclaw:latest"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official Miroclaw image ($fallback_image)"
  ...
  if [[ ${#onboard_cmd[@]} -gt 0 ]]; then
    ...
    info "Docker image ready. Run miroclaw onboard inside the container to configure."
  fi
...
REPO_URL="https://github.com/morpheum-labs/miroclaw.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t miroclaw-bootstrap-XXXXXX)"
...
echo
echo -e "  ${BOLD_BLUE}${CRAB} Miroclaw Installer${RESET}"
...
EXISTING_VERSION=""
INSTALL_MODE="fresh"
if have_cmd miroclaw; then
  EXISTING_VERSION="$(miroclaw --version 2>/dev/null | awk '{print $NF}' || true)"
  INSTALL_MODE="upgrade"
elif [[ -x "$HOME/.cargo/bin/miroclaw" ]]; then
  EXISTING_VERSION="$("$HOME/.cargo/bin/miroclaw" --version 2>/dev/null | awk '{print $NF}' || true)"
  INSTALL_MODE="upgrade"
fi
...
if [[ "$INSTALL_MODE" == "upgrade" && -n "$EXISTING_VERSION" ]]; then
  step_dot "Existing Miroclaw installation detected, upgrading from v${EXISTING_VERSION}"
elif [[ "$INSTALL_MODE" == "upgrade" ]]; then
  step_dot "Existing Miroclaw installation detected, upgrading"
fi
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  echo
  echo -e "${BOLD}Your containerized Miroclaw data is persisted under:${RESET}"
  echo -e "  ${DIM}$DOCKER_DATA_DIR${RESET}"
  echo
  echo -e "${BOLD}Dashboard URL:${RESET} ${BLUE}http://127.0.0.1:42617${RESET}"
  echo
  echo -e "${BOLD}Next steps:${RESET}"
  echo -e "  ${DIM}miroclaw status${RESET}"
  echo -e "  ${DIM}miroclaw agent -m \"Hello, Miroclaw!\"${RESET}"
  echo -e "  ${DIM}miroclaw gateway${RESET}"
  echo
  echo -e "${BOLD}Docs:${RESET} ${BLUE}https://miroclawlabs.ai/docs${RESET}"
  exit 0
fi
...
echo
echo -e "${BOLD_BLUE}[2/3]${RESET} ${BOLD}Installing Miroclaw${RESET}"
if [[ -n "$TARGET_VERSION" ]]; then
  step_dot "Installing Miroclaw v${TARGET_VERSION}"
fi
if [[ "$SKIP_BUILD" == false ]]; then
...
DESKTOP_DOWNLOAD_URL="https://miroclawlabs.ai/download"
...
if [[ "$DEVICE_CLASS" == "desktop" ]]; then
  ...
  case "$OS_NAME" in
    Darwin)
      if [[ -d "/Applications/Miroclaw.app" ]] || [[ -d "$HOME/Applications/Miroclaw.app" ]] \
        || [[ -d "/Applications/ZeroClaw.app" ]] || [[ -d "$HOME/Applications/ZeroClaw.app" ]]; then
        DESKTOP_APP_DETECTED=true
        step_ok "Companion app found (Miroclaw.app or ZeroClaw.app)"
      fi
      ;;
    Linux)
      if have_cmd miroclaw-desktop; then
        DESKTOP_APP_DETECTED=true
        step_ok "Companion app found (miroclaw-desktop)"
      elif [[ -x "$HOME/.local/bin/miroclaw-desktop" ]]; then
        DESKTOP_APP_DETECTED=true
        step_ok "Companion app found (~/.local/bin/miroclaw-desktop)"
      fi
      ;;
  esac
  ...
  if [[ "$DESKTOP_APP_DETECTED" == false ]]; then
    ...
    echo -e "  Menu bar access to your Miroclaw agent."
    ...
    echo -e "  ${DIM}Or run: miroclaw desktop --install${RESET}"
  fi
...
if [[ "$SKIP_ONBOARD" == false && -n "$MIROCLAW_BIN" ]]; then
  if [[ -n "$API_KEY" ]]; then
    ...
    if "${ONBOARD_CMD[@]}" 2>/dev/null; then
      ...
      step_fail "Provider configuration failed — run miroclaw onboard to retry"
    fi
    ...
    if [[ -t 0 && -t 1 ]]; then
      ...
      step_dot "No API key provided — run miroclaw onboard to configure"
    fi
  fi
elif [[ "$SKIP_ONBOARD" == true ]]; then
  step_dot "Skipping configuration (run miroclaw onboard later)"
elif [[ -z "$MIROCLAW_BIN" ]]; then
  warn "Miroclaw binary not found — cannot configure provider"
fi
...
if [[ -n "$MIROCLAW_BIN" ]]; then
  ...
  if "$MIROCLAW_BIN" service install 2>/dev/null; then
    ...
    if "$MIROCLAW_BIN" service restart 2>/dev/null; then
      ...
      step_fail "Gateway service restart failed — re-run with miroclaw service start"
    fi
  else
    step_dot "Gateway service not installed (run miroclaw service install later)"
  fi
...
if [[ -n "$INSTALLED_VERSION" ]]; then
  echo -e "${BOLD_BLUE}${CRAB} Miroclaw installed successfully (Miroclaw ${INSTALLED_VERSION})!${RESET}"
else
  echo -e "${BOLD_BLUE}${CRAB} Miroclaw installed successfully!${RESET}"
fi

if [[ -x "$HOME/.cargo/bin/miroclaw" ]] && ! have_cmd miroclaw; then
  echo
  warn "miroclaw is installed in $HOME/.cargo/bin, but that directory is not in PATH for this shell."
...
if [[ "$DEVICE_CLASS" == "desktop" ]]; then
  if [[ "$DESKTOP_APP_DETECTED" == true ]]; then
    echo -e "  ${DIM}miroclaw desktop${RESET}                ${DIM}# Launch the menu bar app${RESET}"
  else
    echo -e "  ${DIM}miroclaw desktop --install${RESET}      ${DIM}# Download the companion app${RESET}"
  fi
fi
echo
echo -e "${BOLD}Docs:${RESET} ${BLUE}https://miroclawlabs.ai/docs${RESET}"
