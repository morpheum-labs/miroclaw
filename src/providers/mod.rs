...
pub struct ProviderRuntimeOptions {
    ...
    /// These are merged from the config file and `MIROCLAW_EXTRA_HEADERS` env var.
...
/// 3. Generic fallback variables (`MIROCLAW_API_KEY`, `API_KEY`)
///
...
fn resolve_provider_credential(name: &str, credential_override: Option<&str>) -> Option<String> {
    ...
    for env_var in ["MIROCLAW_API_KEY", "API_KEY"] {
        if let Ok(value) = std::env::var(env_var) {
            let value = value.trim();
            if !value.is_empty() {
                return Some(value.to_string());
...
mod tests {
    ...
    fn resolve_provider_credential_opencode_go_env() {
        ...
        let _zeroclaw_guard = EnvGuard::set("MIROCLAW_API_KEY", None);
    ...
    fn env_provider_url_overrides_api_url() {
        std::env::set_var("MIROCLAW_PROVIDER_URL", "http://env-ollama:11434");
        ...
        std::env::remove_var("MIROCLAW_PROVIDER_URL");
    }
}
