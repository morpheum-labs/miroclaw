//! Plugin manifest loader — scans `~/.miroclaw/tools/` at startup.
//!
...
/// Scan `~/.miroclaw/tools/` and return all valid plugins.
///
...
/// Return the path `~/.miroclaw/tools/` using the `directories` crate.
...
"#;
    ...
    fn scan_plugin_dir_skips_broken_manifests_without_panicking() {
        ...
        // always uses ~/.miroclaw/tools), but we can verify load_one_plugin
    ...
    #[test]
    fn plugin_tools_dir_resolves_miroclaw_tools_suffix() {
        let path = plugin_tools_dir().expect("should resolve");
        let display = path.to_string_lossy();
        let expected = std::path::Path::new(crate::context::USER_DATA_DIR_NAME).join("tools");
