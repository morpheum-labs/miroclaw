...
pub struct HardwareBootResult {
    ...
    /// Content of `~/.miroclaw/hardware/` context files (HARDWARE.md, device
    /// profiles, and skills) for injection into the system prompt.
    pub context_files_prompt: String,
}

/// Load hardware context files from `~/.miroclaw/hardware/` and return them
/// concatenated as a single markdown string ready for system-prompt injection.
///
/// Reads (if they exist):
/// 1. `~/.miroclaw/hardware/HARDWARE.md`
/// 2. `~/.miroclaw/hardware/devices/<alias>.md` for each discovered alias
/// 3. All `~/.miroclaw/hardware/skills/*.md` files (sorted by name)
///
...
pub fn load_hardware_context_prompt(aliases: &[&str]) -> String {
    ...
    load_hardware_context_from_dir(
        &crate::context::preferred_user_data_dir_in_home(&home).join("hardware"),
        aliases,
    )
}
...
///
/// Without the feature: loads plugin tools from `~/.miroclaw/tools/` only,
...
mod tests {
    ...
    fn hardware_context_contains_device_exec_rule() {
        ...
        if let Some(home) = directories::BaseDirs::new().map(|d| d.home_dir().to_path_buf()) {
            let hw_md = crate::context::preferred_user_data_dir_in_home(&home)
                .join("hardware")
                .join("HARDWARE.md");
            if hw_md.exists() {
