...
///
/// Checks `MIROCLAW_LOCALE`, then `LANG`, then `LC_ALL`.
/// Returns "en" if none are set or parseable.
pub fn detect_locale() -> String {
    if let Ok(val) = std::env::var("MIROCLAW_LOCALE") {
...
"#,
    ...
    fn detect_locale_from_env() {
        ...
        let saved = std::env::var("MIROCLAW_LOCALE").ok();
        let saved_lang = std::env::var("LANG").ok();

        std::env::set_var("MIROCLAW_LOCALE", "ja-JP");
        assert_eq!(detect_locale(), "ja-JP");

        std::env::remove_var("MIROCLAW_LOCALE");
        std::env::set_var("LANG", "zh_CN.UTF-8");
        assert_eq!(detect_locale(), "zh-CN");

        // Restore.
        match saved {
            Some(v) => std::env::set_var("MIROCLAW_LOCALE", v),
            None => std::env::remove_var("MIROCLAW_LOCALE"),
        }
        match saved_lang {
