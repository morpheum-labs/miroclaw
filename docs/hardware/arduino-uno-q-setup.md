# Miroclaw on Arduino Uno Q — Step-by-Step Guide

Run Miroclaw on the Arduino Uno Q's Linux side. Telegram works over WiFi; GPIO control uses the Bridge (requires a minimal App Lab app).

---

## What's Included (No Code Changes Needed)

Miroclaw includes everything needed for Arduino Uno Q. **Clone the repo and follow this guide — no patches or custom code required.**
...
| Setup command | `src/peripherals/uno_q_setup.rs` | `miroclaw peripheral setup-uno-q` deploys the Bridge via scp + arduino-app-cli |
...
## Phase 2: Install Miroclaw on Uno Q
...
# Clone miroclaw (or scp your project)
...
## Phase 4: Run Miroclaw Daemon

```bash
ssh arduino@<UNO_Q_IP>

# Run daemon (Telegram polling works over WiFi)
miroclaw daemon --host 127.0.0.1 --port 42617
```

**At this point:** Telegram chat works. Send messages to your bot — Miroclaw responds. No GPIO yet.

---

## Phase 5: GPIO via Bridge (Miroclaw Handles It)

Miroclaw includes the Bridge app and setup command.

### 5.1 Deploy Bridge App

**From your Mac** (with miroclaw repo):
```bash
...
Now when you message your Telegram bot *"Turn on the LED"* or *"Set pin 13 high"*, Miroclaw uses `gpio_write` via the Bridge.
...
| 9 | `miroclaw daemon --host 127.0.0.1 --port 42617` |
...
- **"command not found: miroclaw"** — Use full path: `/usr/local/bin/miroclaw` or ensure `~/.cargo/bin` is in PATH.
...
- **GPIO commands ignored** — Ensure Bridge app is running (`miroclaw peripheral setup-uno-q` deploys and starts it). Config must have `board = "arduino-uno-q"` and `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Use `default_provider = "glm"` or `"zhipu"` with `GLM_API_KEY` in env or config. Miroclaw uses the correct v4 endpoint.
