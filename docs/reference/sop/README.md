...
- CLI `miroclaw sop` currently manages definitions only: `list`, `validate`, `show`.
...
2. Create a SOP directory, for example:
   ...
   ```text
   ~/.miroclaw/workspace/sops/deploy-prod/SOP.toml
   ~/.miroclaw/workspace/sops/deploy-prod/SOP.md
   ```
...
3. Validate and inspect definitions:
   ...
   ```bash
   miroclaw sop list
   miroclaw sop validate
   miroclaw sop show deploy-prod
   ```
