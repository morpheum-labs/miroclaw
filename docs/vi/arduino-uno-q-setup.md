# Miroclaw trên Arduino Uno Q — Hướng dẫn từng bước

Chạy Miroclaw trên phía Linux của Arduino Uno Q. Telegram hoạt động qua WiFi; điều khiển GPIO dùng Bridge (yêu cầu một ứng dụng App Lab tối giản).

---

## Những gì đã có sẵn (Không cần thay đổi code)

Miroclaw bao gồm mọi thứ cần thiết cho Arduino Uno Q. **Clone repo và làm theo hướng dẫn này — không cần patch hay code tùy chỉnh nào.**
...
| Setup command | `src/peripherals/uno_q_setup.rs` | `miroclaw peripheral setup-uno-q` triển khai Bridge qua scp + arduino-app-cli |
...
## Phase 2: Cài đặt Miroclaw trên Uno Q
...
# Clone miroclaw (hoặc scp project của bạn)
...
# Cài đặt
sudo cp target/release/miroclaw /usr/local/bin/
```
...
# Copy sang Uno Q
scp target/aarch64-unknown-linux-gnu/release/miroclaw arduino@<UNO_Q_IP>:~/
ssh arduino@<UNO_Q_IP> "sudo mv ~/miroclaw /usr/local/bin/"
```
...
# Cấu hình nhanh
miroclaw onboard --api-key YOUR_OPENROUTER_KEY --provider openrouter
...
## Phase 4: Chạy Miroclaw Daemon

```bash
ssh arduino@<UNO_Q_IP>

# Chạy daemon (Telegram polling hoạt động qua WiFi)
miroclaw daemon --host 127.0.0.1 --port 3000
```

**Tại bước này:** Telegram chat hoạt động. Gửi tin nhắn tới bot — Miroclaw phản hồi. Chưa có GPIO.

---

## Phase 5: GPIO qua Bridge (Miroclaw xử lý tự động)

Miroclaw bao gồm Bridge app và setup command.

### 5.1 Triển khai Bridge App

**Từ Mac** (với repo miroclaw):
```bash
miroclaw peripheral setup-uno-q --host 192.168.0.48
```

**Từ Uno Q** (đã SSH vào):
```bash
miroclaw peripheral setup-uno-q
```
...
Giờ khi bạn nhắn tin cho Telegram bot *"Turn on the LED"* hoặc *"Set pin 13 high"*, Miroclaw dùng `gpio_write` qua Bridge.
...
| 5 | `git clone https://github.com/morpheum-labs/miroclaw.git && cd miroclaw` |
| 6 | `cargo build --release --no-default-features` |
| 7 | `miroclaw onboard --api-key KEY --provider openrouter` |
| 8 | Chỉnh sửa `~/.zeroclaw/config.toml` (thêm Telegram bot_token) |
| 9 | `miroclaw daemon --host 127.0.0.1 --port 3000` |
...
- **"command not found: miroclaw"** — Dùng đường dẫn đầy đủ: `/usr/local/bin/miroclaw` hoặc đảm bảo `~/.cargo/bin` nằm trong PATH.
...
- **Lệnh GPIO bị bỏ qua** — Đảm bảo Bridge app đang chạy (`miroclaw peripheral setup-uno-q` triển khai và khởi động nó). Config phải có `board = "arduino-uno-q"` và `transport = "bridge"`.
- **LLM provider (GLM/Zhipu)** — Dùng `default_provider = "glm"` hoặc `"zhipu"` với `GLM_API_KEY` trong env hoặc config. Miroclaw dùng endpoint v4 chính xác.
